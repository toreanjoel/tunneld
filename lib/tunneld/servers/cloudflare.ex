defmodule Tunneld.Servers.Cloudflare do
  @moduledoc """
  ...
  def handle_cast({:remove_host, subdomain}, state) do
    case Enum.find(state.records, &(&1["subdomain"] == subdomain)) do
      nil ->
        ...
        # Broadcast to notification server
        Tunneld.Servers.Notification.trigger({:critical, "No record #{subdomain} to disconnect"})
      ...
      record ->
        ...
        # Broadcast to notification server
        Tunneld.Servers.Notification.trigger({:info, "Tunnel Disconnected: #{subdomain}"})
  ...
  defp do_add_dns_route(tunnel_name, subdomain) do
    ...
    if exit_code != 0 do
      ...
      # Broadcast to notification server
      Tunneld.Servers.Notification.trigger({:critical, "Erorr adding DNS route for #{subdomain}"})
    else
      ...
      # Broadcast to notification server
      Tunneld.Servers.Notification.trigger({:info, "DNS route added for #{subdomain}"})
    end
  end
  ...
  def get_dns_record_details(domain) do
    [api_key: api_key, zone_id: zone_id] = Application.get_env(:tunneld, :cloudflare)
  ...
  defp tunnel_name_for(subdomain),
    do: "tunneld-" <> String.replace(subdomain, ".", "-")
  ...
  defp config_fs do
    Application.get_env(:tunneld, :fs)
  end
