defmodule TunneldWeb.MachineController do
    ...
    case Tunneld.Machines.enroll(params) do
      {:ok, %{"id" => id, "public_key" => pub, "machine" => machine}} ->
        ...
        |> json(%{
          ...
          public_key: pub,
          instructions:
            "Install this key on the target's ~/.ssh/authorized_keys, then POST /api/v1/machines/#{id}/probe"
        })
  ...
  def probe(conn, %{"id" => id}) do
    case Tunneld.Machines.probe(id) do
      ...
      {:error, reason} ->
        conn |> put_status(502) |> json(%{error: "probe failed", detail: inspect(reason)})
    end
  end
  ...
  def listeners(conn, %{"id" => id}) do
    case Tunneld.Machines.listeners(id) do
      {:ok, listeners} ->
        json(conn, %{listeners: listeners})

      {:error, :not_found} ->
        conn |> put_status(404) |> json(%{error: "not found"})

      {:error, reason} ->
        conn |> put_status(502) |> json(%{error: "list failed", detail: inspect(reason)})
    end
  end
